...
const categoryKeyMap = {
  ...
  'fűzésmód':'fűzésmód','kötésmód':'kötésmód',
  'borítószerkezet':'borítószerkezet','borító':'borítószerkezet',
  'gerincszerkezet':'gerincszerkezet',
  'anyag':'anyag','material':'anyag',
  'díszítés':'díszítés','diszites':'díszítés','decoration':'díszítés',
...
const categoryLabelMap = {
  ...
  'fűzésmód':'Fűzésmód',
  'kötésmód':'Kötésmód',
  'borítószerkezet':'Borítószerkezet',
  'gerincszerkezet':'Gerincszerkezet',
  'anyag':'Anyag',
  'díszítés':'Díszítés',
  'záródás':'Záródás',
  'tárolóelem':'Tárolóelem'
};
...
const categoryColors = {
  ...
  'fűzésmód':'#c2c9ef',
  'kötésmód':'#dbb8e3',
  'borítószerkezet':'#e6d8b8',
  'gerincszerkezet':'#88c6c3',
  'anyag':'#d9d9d9',
  'díszítés':'#f0b36a',
  'záródás':'#c4ca4e',
  'tárolóelem':'#eecba4'
...
const state = {
  ...
  search: '',
  view: 'cards',
  openCategories: new Set(['formátum'])
};
...
function formatInline(value){
  return escapeHtml(value).replace(/\[([^\]]+)\]/g,'<span class="bracket-note">[$1]</span>');
}
...
function displayCategory(category){
  return categoryKeyMap[String(category || '').toLowerCase()] || String(category || '').toLowerCase();
}
...
function getSearchText(term){
  return normalize([
    term.id, term.prefLabelHu, term.prefLabelEn, term.category, term.class, term.property,
    term.cidoc, term.definition, term.usage, term.meaning, term.demoStatus,
    ...term.altLabelHu, ...term.altLabelEn, ...term.relatedTerms, ...term.sources
  ].join(' '));
}

function byCategoryThenLabel(a,b){
  const ai = categoryOrder.indexOf(displayCategory(a.category));
  const bi = categoryOrder.indexOf(displayCategory(b.category));
  const diff = (ai < 0 ? 999 : ai) - (bi < 0 ? 999 : bi);
  return diff || String(a.prefLabelHu).localeCompare(String(b.prefLabelHu),'hu');
}

function getVisibleTerms(){
  const query = normalize(state.search.trim());
  return state.terms.filter(term => !query || getSearchText(term).includes(query)).sort(byCategoryThenLabel);
}
...
function getFilteredTerms(){
  ...
  return state.selectedCategory === 'összes'
    ? visible
    : visible.filter(term => displayCategory(term.category) === state.selectedCategory);
}

function tagStyle(category,term){
  const color = term.tagColor || categoryColors[category] || '';
  return color ? ` style="background:${escapeHtml(color)};border-color:${escapeHtml(color)}"` : '';
}

function renderTags(items){
  const values = asArray(items);
  return values.map(item => `<button class="tag" type="button" data-tag="${escapeHtml(item)}">${formatInline(item)}</button>`).join('');
}

function renderSourceItems(items){
  const values = asArray(items);
  if(!values.length) return '<span class="source-item">—</span>';
  return values.map(item => `<span class="source-item">${formatInline(item)}</span>`).join('');
}

function field(label,value){
  return `<div class="field"><strong>${escapeHtml(label)}</strong>${formatInline(value || '—')}</div>`;
}

function tagField(label,values){
  const valuesArray = asArray(values);
  if(!valuesArray.length) return '';
  return `<div class="field"><strong>${escapeHtml(label)}</strong><div class="tag-list">${renderTags(valuesArray)}</div></div>`;
}
...
function noteParagraph(value){
  return value ? `<p class="detail-paragraph">${formatInline(value)}</p>` : '';
}

function statusField(label,value){
  return `<div class="field technical-field"><strong>${escapeHtml(label)}</strong><span class="status-badge">${formatInline(value || 'pilot')}</span></div>`;
}

function detailSection(title,content){
  return content ? `<section class="detail-section"><h4>${escapeHtml(title)}</h4>${content}</section>` : '';
}
...
function getCardMarkup(term){
  ...
  const usageContent = `${noteParagraph(term.usage)}${noteParagraph(term.meaning)}`;
  const relationContent = `${tagField('Alternatív elnevezések',term.altLabelHu)}${tagField('Kapcsolódó fogalmak',term.relatedTerms)}`;

  return `<article class="card" data-category="${escapeHtml(category)}" data-term-id="${escapeHtml(term.id)}">
    <div class="card-top">
      <span class="category" data-category="${escapeHtml(category)}"${tagStyle(category,term)}>${escapeHtml(label)}</span>
      <h3>${formatInline(term.prefLabelHu)}</h3>
      <div class="en">${formatInline(term.prefLabelEn)}</div>
    </div>
    <div class="card-body">
      <p class="definition">${formatInline(term.definition)}</p>
      <details>
        <summary><span>Részletes nézet</span><span class="summary-icon" aria-hidden="true">↓</span></summary>
        <div class="detail-block">
          ${detailSection('Használat és értelmezés',usageContent)}
          ${detailSection('Terminológiai kapcsolatok',relationContent)}
          <section class="detail-section technical-data">
            <h4>Technikai adatok</h4>
            ${technicalGroup('Rekord',`<div class="ontology-row">${field('Azonosító',term.id)}${statusField('Státusz',term.demoStatus)}</div>`)}
...
function renderCategoryBrowse(){
  els.filters.innerHTML = categoryOrder.map(category => {
    ...
    return `<section class="browse-group" data-category="${escapeHtml(category)}">
      <button class="browse-toggle ${isOpen ? 'open' : ''}" type="button" data-category="${escapeHtml(category)}" aria-expanded="${isOpen}">
        <span>${isOpen ? '▾' : '▸'} ${escapeHtml(meta.title)}</span><span class="count">${terms.length}</span>
      </button>
      <div class="browse-terms ${isOpen ? 'open' : ''}">
        ${terms.map(term => `<button class="browse-term ${term.id === state.selectedTerm ? 'active' : ''}" type="button" data-id="${escapeHtml(term.id)}">${formatInline(term.prefLabelHu)}</button>`).join('')}
      </div>
    </section>`;
  }).join('');

  els.filters.querySelectorAll('.browse-toggle').forEach(button => button.addEventListener('click',() => {
  ...
  els.filters.querySelectorAll('.browse-term').forEach(button => button.addEventListener('click',() => {
    const term = state.terms.find(item => item.id === button.dataset.id);
    if(!term) return;
    state.selectedCategory = displayCategory(term.category);
    state.selectedTerm = term.id;
    state.openCategories.add(state.selectedCategory);
    renderAll();
...
function renderCategoryPanels(){
  ...
  els.categoryFolders.innerHTML = categoryOrder.map(category => {
    const meta = categoryMeta[category];
    const count = getCategoryTerms(category).length;
    return `<button class="category-panel ${category === state.selectedCategory ? 'active' : ''}" type="button" data-category="${escapeHtml(category)}">
      <h3 class="category-panel-title">${escapeHtml(meta.title)}</h3>
      <span class="category-panel-media"><img src="${escapeHtml(meta.icon)}" alt="" loading="lazy"></span>
      <span class="category-panel-text"><p class="category-panel-description">${escapeHtml(meta.description)}</p></span>
  ...
  els.categoryFolders.querySelectorAll('button').forEach(button => button.addEventListener('click',() => {
    state.selectedCategory = button.dataset.category;
    state.openCategories.add(state.selectedCategory);
    renderAll();
    if(els.termsTopline) els.termsTopline.scrollIntoView({behavior:'smooth',block:'start'});
  }));
}
...
async function loadJson(url){
  ...
  if(!response.ok) throw new Error(`Nem tölthető be: ${url}`);
  return await response.json();
}

async function loadDefinitionOverrides(){
  try{
    const response = await fetch('data/definition-overrides.json?v=colors-4');
...
async function init(){
  try{
    ...
    const [master,decorations,definitionOverrides] = await Promise.all([
      loadJson(masterUrl),
      loadJson('data/decorations.json?v=1'),
      loadDefinitionOverrides()
    ]);
    const allTerms = [...(master.terms || []), ...(decorations.terms || [])];
    state.terms = allTerms
      .map(normalizeTerm)
      .map(term => ({...term, definition: definitionOverrides[term.id] || term.definition}));
    renderAll();
  }catch(error){
...
function setSearch(value){
  ...
  if(els.quickSearchInput) els.quickSearchInput.value = value;
  renderAll();
}
